...
public:
    ...
    virtual void            Set( unsigned long seed = 0 ) = 0;

    unsigned long           GetI( unsigned long interval );
    double                  GetDouble0();
    double                  GetDouble1();
    double                  GetDouble01();

    unsigned long           GetMin() const { return min_; }
    unsigned long           GetMax() const { return max_; }
    unsigned int            GetSize() const { return size_; }

protected:
    void    SetMin( unsigned long value ) { min_ = value; }
    void    SetMax( unsigned long value ) { max_ = value; }
...
public:
    ...
    virtual void            Set( unsigned long seed = 0 );

    static const unsigned long  GetDefSeed() { return sD_seed; }

protected:
    static unsigned long    MAGIC( unsigned long y ) { return ( y & 1 )? 0x9908b0dfUL: 0; }
    static unsigned long    GetMaskUPPER() { return sM_UPPER; }
    static unsigned long    GetMaskLOWER() { return sM_LOWER; }

private:
    unsigned long   mt_[P1];
    int             mti_;
    static const unsigned long  sM_UPPER;
    static const unsigned long  sM_LOWER;
    static const unsigned long  sD_seed;
};
...
protected:
    static unsigned long    LCG( unsigned long s ) { return ( 69069 * s )& 0xffffffffUL; }

private:
    unsigned long   rs_[P1];//register sequence
    int             ri_;
    static const unsigned long  sD_seed;
};
...
public:
    ...
    virtual unsigned long   Get();
    virtual double          GetDouble();
    virtual void            Set( unsigned long seed = 0 ) { Set1( seed ); }
    void    Set1( unsigned long seed = 0 ) { SetLux( seed, 202 ); }
    void    Set2( unsigned long seed = 0 ) { SetLux( seed, 397 ); }

    static const unsigned long  GetDefSeed() { return sD_seed; }

protected:
    void    SetLux( unsigned long seed, unsigned int luxury );
    void    Increment();
    void    RANLUXstep( double&, double&, unsigned int, unsigned int, unsigned int );
...
//
inline
void RANLUXdRng::RANLUXstep( double& r1, double& r2, 
