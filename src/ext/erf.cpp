...
//
static int erfseries( double x, double* result, double* err )
{
    double  coef = x;
    double  e = coef;
    double  del;
    ...
    if( result ) {
        *result = 2.0 / SLC_SQRTPI * e;
        if( err )
            *err = 2.0 / SLC_SQRTPI * ( fabs( del ) + SLC_DBL_EPSILON );
    }
    return PSL_SUCCESS;
}
...
static Tcheb_series erfc_xlt1_cs = {
    erfc_xlt1_data,
    19,
    -1, 1,
    12
...
static Tcheb_series erfc_x15_cs = {
    erfc_x15_data,
    24,
    -1, 1,
    16
...
static Tcheb_series erfc_x510_cs = {
    erfc_x510_data,
    19,
    -1, 1,
    12
...
int psl_erfc_e( double x, double* result, double* err  )
{
    ...
    if( ax <= 1.0 ) {
        double  t = 2.0 * ax - 1.0;
        double  c, cerr;
        cheb_eval_e( &erfc_xlt1_cs, t, &c, &cerr );
        e_val = c;
        e_err = cerr;
    }
    else if( ax <= 5.0 ) {
        ...
        double  t = 0.5 * ( ax - 3.0 );
        double  c, cerr;
        cheb_eval_e( &erfc_x15_cs, t, &c, &cerr );
        e_val = ex2 * c;
        e_err = ex2 *( cerr + 2.0 * fabs( x ) * SLC_DBL_EPSILON );
    }
    else if( ax < 10.0 ) {
        double  exterm = exp( -x * x ) / ax;
        double  t = ( 2.0 * ax - 15.0 )/ 5.0;
        double  c, cerr;
        cheb_eval_e( &erfc_x510_cs, t, &c, &cerr );
        e_val = exterm * c;
        e_err = exterm *( cerr + 2.0 * fabs( x ) * SLC_DBL_EPSILON + SLC_DBL_EPSILON );
    }
    else {
        e_val = erfc8( ax );
        e_err = ( x * x + 1.0 ) * SLC_DBL_EPSILON * fabs( e_val );
    }
...
{
    if( x * x < 10.0 * SLC_ROOT6_DBL_EPSILON ) {
        const double y = x / SLC_SQRTPI;
        // * series for -1/2 Log[Erfc[Sqrt[Pi] y]]
        const double c3 = ( 4.0 - SLC_PI )/ 3.0;
        const double c4 = 2.0 *( 1.0 - SLC_PI / 3.0 );
...
//     }
        ...
        double  result_erfc, result_erfc_err;
        psl_erfc_e( x, &result_erfc, &result_erfc_err );
        if( result ) {
            *result = log( result_erfc );
            if( err ) {
                *err = fabs( result_erfc_err / result_erfc );
                *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
            }
        }
        return PSL_SUCCESS;
...
{
    if( fabs( x ) < 1.0 ) {
        return erfseries( x, result, err );
    }
    else {
        double  result_erfc, result_erfc_err;
        psl_erfc_e( x, &result_erfc, &result_erfc_err );
        if( result ) {
            *result = 1.0 - result_erfc;
            if( err ) {
                *err = result_erfc_err;
                *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
            }
...
// -------------------------------------------------------------------------
// Standard Gaussian probability density function, (1/sqrt(2*pi)) exp(-x^2/2)
//
int psl_erf_Z_e( double x, double* result, double* err )
{
    const double ex2 = exp( -x * x /2.0 );
    if( result ) {
        *result = ex2 /( SLC_SQRT2 * SLC_SQRTPI );
        if( err ) {
            *err = fabs( x * *result ) * SLC_DBL_EPSILON;
            *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
        }
...
{
    double  result_erfc, result_erfc_err;
    int     status = psl_erfc_e( x / SLC_SQRT2, &result_erfc, &result_erfc_err );
    if( result ) {
        *result = 0.5 * result_erfc;
        if( err ) {
            *err = 0.5 * result_erfc_err;
            *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
        }
    }
    return status;
...
// -------------------------------------------------------------------------
// The hazard function of the standard normal distribution, also 
// known as the inverse Mill's ratio, Z(x)/Q(x);
// decreases rapidly as x->-oo, and ~x as x->+oo
//
int psl_hazard_e( double x, double* result, double* err )
{
    if( x < 25.0 ) {
        double  result_ln_erfc, result_ln_erfc_err;
        const int lstatus = psl_log_erfc_e( x / SLC_SQRT2, &result_ln_erfc, &result_ln_erfc_err );
        const double lnc = -0.22579135264472743236; // * ln(sqrt(2/pi))
        const double arg = lnc - 0.5 * x * x - result_ln_erfc;
        const int estatus = psl_exp_e( arg, result, err );
        if( err ) {
            *err += 3.0 *( 1.0 + fabs( x )) * SLC_DBL_EPSILON * fabs( *result );
            *result += fabs( result_ln_erfc_err * *result );
        }
        if( lstatus != PSL_SUCCESS )
            return lstatus;
