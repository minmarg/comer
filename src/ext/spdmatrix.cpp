...
void SPDmatrix::priverror( const char* errstr ) const
{
    fprintf( stderr, "ERROR: SPDmatrix: %s.\n", errstr );
    abort();
}
...
int SPDmatrix::CholeskyDecompose()
{
    ...
    if( norows != nocols ) {
        ...
        fprintf( stderr, "PSL_ERR_DIM: CholeskyDecompose.\n");
#endif
        return PSL_ERR_DIM;
    }
    ...
    if( A_00 <= 0.0 ) {
#ifdef SPDmatrixTESTPRINT
        fprintf( stderr, "PSL_ERR_DOMAIN: CholeskyDecompose: %s.\n", errdom );
#endif
...
#ifdef SPDmatrixTESTPRINT
                    fprintf( stderr, "PSL_ERR_DIM: CholeskyDecompose: Dot product failed.\n" );
#endif
                    return PSL_ERR_DIM;
                }
            }
...
#ifdef SPDmatrixTESTPRINT
        fprintf( stderr, "PSL_ERR_DIM: CholeskySolve.\n");
#endif
        return PSL_ERR_DIM;
    }
...
#ifdef SPDmatrixTESTPRINT
        fprintf( stderr, "CholeskySolve: %s\n", TranslatePSLError( status ));
#endif
        return status;
    }
...
//  On input this matrix is assumed to have already decomposed: A = L L^T.
...
int SPDmatrix::CDedInvert()
{
    ...
    if( norows != nocols ) {
#ifdef SPDmatrixTESTPRINT
        fprintf( stderr, "PSL_ERR_DIM: CholeskyInvert.\n");
#endif
        return PSL_ERR_DIM;
    }
    ...
    for( i = 0; i < norows; i++ ) {
        ...
        if( A_jj == 0.0 ) {
#ifdef SPDmatrixTESTPRINT
            fprintf( stderr, "PSL_ERR_ILLEGAL: CholeskyInvert.\n");
#endif
...
//
int SPDmatrix::CDedDet( double* det ) const
{
    if( !det )
        return 1;
...
//  is equal to the squared product of the diagonal elements.
//
int SPDmatrix::CDedLogDet( double* ldet ) const
{
    if( !ldet )
        return 1;
