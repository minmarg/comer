...
//
inline
static int riemann_zeta_sgt0( double s, double* result, double* err )
{
    if( s < 1.0 ) {
        double  c, cerr;
        cheb_eval_e( &zeta_xlt1_cs, 2.0 * s - 1.0, &c, &cerr );
        if( result ) {
            *result = c /( s - 1.0 );
            if( err )
                *err = cerr / fabs( s - 1.0 ) + SLC_DBL_EPSILON * fabs( *result );
        }
        return PSL_SUCCESS;
    }
    else if( s <= 20.0 ) {
        ...
        if( result ) {
            *result = c /( s - 1.0 );
            if( err )
                *err = cerr /( s - 1.0 ) + SLC_DBL_EPSILON * fabs( *result );
        }
        return PSL_SUCCESS;
    }
    else {
        ...
        if( result ) {
            *result = 1.0/( f2 * f3 * f5 * f7 );
            if( err )
                *err = 3.0 * SLC_DBL_EPSILON * fabs( *result );
        }
        return PSL_SUCCESS;
    }
}
...
//
inline
static int riemann_zeta1ms_slt0( double s, double* result, double* err )
{
    if( -19.0 < s ) {
        ...
        if( result ) {
            *result = c /( -s );
            if( err )
                *err = cerr /( -s ) + SLC_DBL_EPSILON * fabs( *result );
        }
        return PSL_SUCCESS;
    }
    else {
...
// works for 5 < s < 15
//
static int riemann_zeta_minus_1_intermediate_s( double s, double* result, double* err )
{
    double  t = ( s - 10.0 )/ 5.0;
    double  c, cerr;
    cheb_eval_e( &zetam1_inter_cs, t, &c, &cerr );
    if( result ) {
        *result = exp( c ) + pow( 2.0, -s );
        if( err )
            *err = ( cerr + 2.0 * SLC_DBL_EPSILON ) * *result;
    }
    return PSL_SUCCESS;
}
...
// works well for s > 15
//
static int riemann_zeta_minus1_large_s( double s, double* result, double* err )
{
    double  a = pow( 2.0, -s );
    double  b = pow( 3.0, -s );
    double  c = pow( 5.0, -s );
    ...
    */
    ...
    if( result ) {
        *result = numt * zeta;
        if( err )
            *err = ( 15.0 / s + 1.0 ) * 6.0 * SLC_DBL_EPSILON * *result;
    }
    return PSL_SUCCESS;
}
...
int psl_hzeta_e( const double s, const double q, double* result, double* err )
{
    if( s <= 1.0 || q <= 0.0 ) {
        ...
        if( ln_term0 < SLC_LOG_DBL_MIN + 1.0 ) {
            return PSL_ERR_UNDERFLOW;
        }
        else if( SLC_LOG_DBL_MAX - 1.0 < ln_term0 ) {
            ...
            if( result ) {
                *result = p1 * ( 1.0 + p2 + p3 );
                if( err )
                    *err = SLC_DBL_EPSILON * ( 0.5 * s + 2.0 ) * fabs( *result );
            }
            return PSL_SUCCESS;
        }
        else {
            ...
            for( j = 0; j <= jmax; j++ ) {
                ...
                ans += delta;
                if( fabs( delta / ans ) < 0.5 * SLC_DBL_EPSILON )
                    break;
                scp *= ( s + 2*j + 1 )*( s + 2*j + 2);
                pcp /= ( kmax + q )*( kmax + q );
            }
            if( result ) {
                *result = ans;
                if( err )
                    *err = 2.0 * ( jmax + 1.0 ) * SLC_DBL_EPSILON * fabs( ans );
            }
            return PSL_SUCCESS;
        }
    }
...
int psl_zeta_e( const double s, double* result, double* err )
{
    if( s == 1.0 ) {
        ...
        double  zeta_one_minus_s, zeta_one_minus_s_err;
        const int zomstatus = riemann_zeta1ms_slt0( s, &zeta_one_minus_s, &zeta_one_minus_s_err );
        const double sin_term = ( fmod( s, 2.0 ) == 0.0 )? 0.0: sin( 0.5 * SLC_PI * fmod( s, 4.0 ))/ SLC_PI;
        ...
        if( sin_term == 0.0 ) {
            ...
            const double  p = pow( 2.0 * SLC_PI, fs ) / twopi_pow[n];

            double  g, gerr;
            const int gstatus = psl_gamma_e( 1.0 - s, &g, &gerr );
            if( result ) {
                *result = p * g * sin_term * zeta_one_minus_s;
                if( err ) {
                    *err = fabs( p * g * sin_term ) * zeta_one_minus_s_err;
                    *err += fabs( p * sin_term * zeta_one_minus_s ) * gerr;
                    *err += SLC_DBL_EPSILON *( fabs( s ) + 2.0 ) * fabs( *result );
                }
            }
...
int psl_zeta_int_e( const int n, double* result, double* err )
{
    if( n < 0 ) {
        if( !SLC_ODD( n )) {
            ...
            return psl_zeta_e(( double )n, result, err );
        }
    }
    else if( n == 1 ) {
        return PSL_ERR_DOMAIN;
        ...
        if( result ) {
            *result = 1.0 + zetam1_pos_int_table[n];
            if( err )
                *err = 2.0 * SLC_DBL_EPSILON * fabs( *result );
        }
        return PSL_SUCCESS;
...
int psl_zetam1_int_e( const int n, double* result, double* err )
{
    if( n < 0 ) {
        if( !SLC_ODD( n )) {
            if( result ) {
                *result = -1.0; // at even negative integers zetam1 == -1 since zeta is exactly zero
                if( err )
                    *err = 0.0;
            }
            return PSL_SUCCESS;
            ...
            if( result ) {
                *result = zeta_neg_int_table[-(n+1)/2] - 1.0;
                if( err )
                    *err = 2.0 * SLC_DBL_EPSILON * fabs( *result );
            }
            return PSL_SUCCESS;
        ...
        return psl_zetam1_e(( double )n, result, err );
    }
}
...
int psl_eta_int_e( int n, double* result, double* err )
{
    if( ETA_POS_TABLE_NMAX < n ) {
        ...
        if( !SLC_ODD( n )) {
            ...
            int     pstatus = psl_exp_e(( 1.0 - n ) * SLC_LN2, &p, &perr );
            int     mstatus = psl_multiply_e( -p, z, result, err );
            if( err ) {
                *err = fabs( perr *( SLC_LN2 *( 1.0 - n )) * z ) + zerr * fabs( p );
                *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
            }
...
int psl_eta_e( const double s, double* result, double* err )
{
    if( 100.0 < s ) {
    ...
    else if( fabs( s - 1.0 ) < 10.0 * SLC_ROOT5_DBL_EPSILON ) {
        double  del = s - 1.0;
        double  c0  = SLC_LN2;
        double  c1  = SLC_LN2 * ( SLC_EULER - 0.5 * SLC_LN2 );
        ...
        int     pstatus = psl_exp_e(( 1.0 - s ) * SLC_LN2, &p, &perr );
        int     mstatus = psl_multiply_e( 1.0 - p, z, result, err );
        if( err ) {
            *err = fabs( perr *( SLC_LN2 *( 1.0 - s )) * z ) + zerr * fabs( p );
            *err += 2.0 * SLC_DBL_EPSILON * fabs( *result );
        }
        if( mstatus != PSL_SUCCESS )
            return mstatus;
