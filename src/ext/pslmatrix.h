...
#ifndef __pslmatrix__
#define __pslmatrix__
...
public:
    ...
    int         Add( double );
    int         AddToDiag( double value );
    int         AddIdentity() { return AddToDiag( 1.0 ); }
    ...
    double      GetValueAt( int n, int m ) const;   //get value at the position
    void        SetValueAt( int n, int m, double value );//set value at the position
    void        AddValueAt( int n, int m, double value );//add value at the position
    void        MulValueAt( int n, int m, double value );//multiply by value at the position
...
protected:
    ...
    const double*   GetValues() const { return values_; }
...
protected:
    double*     values_;        //double values of matrix
...
//
inline
double Pslmatrix::GetValueAt( int n, int m ) const
{
#ifdef __DEBUG__
...
//
inline
void Pslmatrix::SetValueAt( int n, int m, double value )
{
#ifdef __DEBUG__
    if( !values_ || norows_ <= n || nocols_ <= m || rowdim_ < 1 )
        priverror( "Memory access error" );
...
//
inline
void Pslmatrix::AddValueAt( int n, int m, double value )
{
#ifdef __DEBUG__
    if( !values_ || norows_ <= n || nocols_ <= m || rowdim_ < 1 )
        priverror( "Memory access error" );
...
//
inline
void Pslmatrix::MulValueAt( int n, int m, double value )
{
#ifdef __DEBUG__
    if( !values_ || norows_ <= n || nocols_ <= m || rowdim_ < 1 )
        priverror( "Memory access error" );
