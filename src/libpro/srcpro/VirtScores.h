...
#ifndef __VirtScores__
#define __VirtScores__
...
// -------------------------------------------------------------------------
// class VirtScores: Secondary structure state scores
//
class VirtScores
{
public:
...
//  eff. no seqns
inline
Pslvector* VirtScores::GetScores( int plvl, int lvl )
{
...
//  eff. no seqns
inline
const Pslvector* VirtScores::GetScores( int plvl, int lvl ) const
{
    if( plvl < 0 || noplvs_ <= plvl )
        throw myruntime_error("VirtScores: GetScores: Memory access error.");
    if( lvl < 0 || notbls_ <= lvl )
...
inline
double VirtScores::GetScore( int row, int col, 
    double fstprb, double secprb, double fstens, double secens ) const
{
    ...
    if( nolvs < 1 )
        throw myruntime_error("VirtScores: GetScore: No levels of eff. no. sqns.");
