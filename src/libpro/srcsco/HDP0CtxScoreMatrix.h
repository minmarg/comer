...
#ifndef __HDP0CtxScoreMatrix__
#define __HDP0CtxScoreMatrix__
...
class HDP0CtxScoreMatrix: public AbstractScoreMatrix
{
public:
    HDP0CtxScoreMatrix(
    ...
    explicit HDP0CtxScoreMatrix();
    virtual ~HDP0CtxScoreMatrix();
    virtual double      GetScore( int m, int n ) const;             //get score at the positions
    virtual const char* GetMethodName() const   { return "Profile-specific HDP0Ctx-based"; }
...
inline
double HDP0CtxScoreMatrix::GetScore( int m, int n ) const
{
...
inline
bool HDP0CtxScoreMatrix::ThicknessConstraintsMet( int m, int n ) const
{
#ifdef __DEBUG__
    if( GetSubjectSize() <= m || m < 0 )
        throw myruntime_error( mystring( "HDP0CtxScoreMatrix: Memory access error." ));

    if( GetQuerySize() <= n || n < 0 )
        throw myruntime_error( mystring( "HDP0CtxScoreMatrix: Memory access error." ));

    if( !logo_sec_.GetEffNoSequences() || !logo_fst_.GetEffNoSequences())
        throw myruntime_error( mystring( "HDP0CtxScoreMatrix: Profile thickness found to be wrong." ));
#endif
...
//
inline
void HDP0CtxScoreMatrix::InitHDPbase()
{
    DestroyHDPbase();
    hdpbase_ = new HDPbase();
    if( hdpbase_ == NULL )
        throw myruntime_error("HDP0CtxScoreMatrix: Not enough memory.");
}
...
//
inline
void HDP0CtxScoreMatrix::InitQueryBasin()
{
    DestroyQueryBasin();
...
//
inline
void HDP0CtxScoreMatrix::InitSbjctBasin()
{
    DestroySbjctBasin();
