...
//  (the code taken from the NCBI BLAST toolbox sources)
int PC_Gcd( int n, int d )
{
    int t;
...
// W.H.Press's et al. "Numerical Recipes in C".

double LecuyerRand( long* idum )
{
    int         j;
    long        k;
        ...
        double* result )
{
    ...
    if(( fl > 0.0 && fh > 0.0 ) || ( fl < 0.0 && fh < 0.0 ))
        return( "Root evaluation: Root must be bracketed in interval supplied." );
