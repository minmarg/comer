...
// -------------------------------------------------------------------------
// GetRestaurantAt: get restaurant at the position
//
inline
...
// -------------------------------------------------------------------------
// SetRestaurantAt: set restaurant at the position
//
inline
...
#ifdef __DEBUG__
    if( !values_ || loc < 0 || length_ <= loc )
        throw myruntime_error( "RntChain: Memory access error" );
#endif
    values_[loc] = value;
}
...
int RntChain::NewRestaurant( Restaurant* value )
{
    ...
    if( 0 < novacs_ ) {
        ...
        if( loc < 0 || length_ <= loc )
            throw myruntime_error( "RntChain: NewValue: Memory access error" );
        values_[loc] = value;
        novacs_--;
        actlen_++;
        return loc;
...
// -------------------------------------------------------------------------
// RemRestaurantAt: remove restaurant at the position
//
inline
