...
// -------------------------------------------------------------------------
// class Dish: reusable vector of values (objects)
//
class Dish
{
public:
    ...
    int         SetReadSize( int value ) { prosize_ = value; }

    int         GetReadNoTables() const { return pronotbls_; }
    int         SetReadNoTables( int value ) { pronotbls_ = value; }
    ...
    double      GetTmpValue() const { return tmpval_; }
    void        SetTmpValue( double value ) { tmpval_ = value; }
...
private:
    time_t      time_;          //time dish created
    ...
    int         notables_;      //number of tables having dish k
    double      tmpval_;        //temporary value used to store probability
};
...
//
inline
Pslvector* Dish::GetVectorNAt( int loc ) const
{
...
//
inline
void Dish::RemValueAt( int loc, const Pslvector* value )
{
#ifdef __DEBUG__
    if( !values_ || loc < 0 || length_ <= loc )
